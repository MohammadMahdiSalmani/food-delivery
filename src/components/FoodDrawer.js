...
const FoodDrawer = () => {
  ...
  return (
    <Drawer open={food.status} size={width > 421 ? 'large' : 'default'} onClose={() => dispatch(toggleDrawer({ status: false, name: "", price: "", material: "", count: 0, img: "" }))} placement={width > 421 ? 'left' : 'bottom'} closable={false} className={`flex flex-col ${width > 421 ? 'rounded-r-2xl' : 'rounded-t-2xl'}`}>
      <div className='absolute -translate-y-1/2 top-1/2'>
        ...
        <label className='flex justify-center items-center text-center px-16 mb-5 text-lg text-gray'>{food.material}</label>
        ...
        <div className='flex justify-center mt-10'>
          ...
          <Button onClick={() => dispatch(increment({ name: food.name, price: food.price, material: food.material, img: food.img }))} type="primary" className="border-2 border-solid bg-primary">+</Button>
        </div>
      </div>
    </Drawer>
