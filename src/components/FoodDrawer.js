...
const FoodDrawer = () => {
  ...
  const count = cart.find(item => item.name === food.name)
    ...
    <Drawer open={food.status} onClose={() => dispatch(toggleDrawer({ status: false, name: "", price: "", count: 0, img: "" }))} placement='bottom' closable={false} className='flex flex-col rounded-t-3xl'>
      ...
      <div className='flex justify-center mt-10'>
        <>
          ...
          <Button onClick={() => dispatch(increment({ name: food.name, price: food.price }))} type="primary" className="border-2 border-solid bg-primary">+</Button>
        </>
      </div>
    </Drawer>
